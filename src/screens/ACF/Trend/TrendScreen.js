...
export default function TrendScreen() {
  const { listQuery } = useTrendingPosts();
  const {
    data,
    isLoading,
    isFetchingNextPage,
  ...
  const renderEmpty = useMemo(
    () => (
      <EmptyState
        ...
        description="Khi có bài viết nổi bật, chúng sẽ hiển thị tại đây."
      />
    ),
    [],
  );
  ...
  const listHeader = useMemo(
    () => (
      <View style={{ gap: gapSmall }}>
        ...
        >
          Tổng cộng {totalPosts} bài viết đang được quan tâm nhiều nhất.
        </Text>
      </View>
    ),
  ...
  if (isLoading) {
    return <LoadingSpinner message="Đang tải bài viết xu hướng..." />;
  }
